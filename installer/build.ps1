...
if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT) {
    throw "Run this build on 64-bit Windows 10/11."
}
...
function Find-BuildPython([string]$privatePath) {
    ...
    foreach ($candidate in $candidates | Select-Object -Unique) {
        ...
        try {
            $validated = & $candidate -c "import sys, venv, tkinter, struct; assert sys.version_info[:2] == (3, 12) and struct.calcsize('P') == 8; print(sys.executable)" 2>$null | Select-Object -Last 1
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Reader tests failed." }
    & $venvPython -m PyInstaller --noconfirm --clean LineReader.spec
    if ($LASTEXITCODE -ne 0) { throw "PyInstaller build failed." }
    $smokeLog = Join-Path $toolsDir ("smoke-test-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss-fff"))
    $smoke = New-Object System.Diagnostics.Process
    $smoke.StartInfo.FileName = Join-Path $root "dist\LineReader\LineReader.exe"
    $smoke.StartInfo.WorkingDirectory = $root
    $smoke.StartInfo.UseShellExecute = $false
    $smoke.StartInfo.Arguments = '--smoke-test --smoke-log "{0}"' -f $smokeLog
    try {
        if (-not $smoke.Start()) { throw "Unable to start packaged application." }
        if (-not $smoke.WaitForExit(120000)) {
            $smoke.Kill()
            throw "Packaged application smoke test timed out. Log: $smokeLog"
        }
        $smokeExitCode = $smoke.ExitCode
        $smokeText = ""
        if (Test-Path $smokeLog) {
            $smokeText = [System.IO.File]::ReadAllText($smokeLog, [System.Text.Encoding]::UTF8)
            Write-Host $smokeText
        }
        if ($smokeExitCode -ne 0 -or $smokeText -notmatch '(?m)^PASS\r?$') {
            throw "Packaged application smoke test failed. Exit code: $smokeExitCode. Log: $smokeLog"
        }
    } finally {
        $smoke.Dispose()
    }
...
$issFile = Join-Path $installerDir "LineReader.iss"
$issText = [System.IO.File]::ReadAllText($issFile)
$versionMatch = [regex]::Match($issText, '(?m)^\s*#define\s+MyAppVersion\s+"([^"]+)"')
if (-not $versionMatch.Success) { throw "MyAppVersion was not found in LineReader.iss." }
$appVersion = $versionMatch.Groups[1].Value

& $iscc $issFile
if ($LASTEXITCODE -ne 0) { throw "Inno Setup build failed." }

$output = Join-Path $root ("output\LineReader-Setup-{0}.exe" -f $appVersion)
